...
module.exports.repost = (req, res, next) => {
    let {
        repostTo,
        post
    } = req.body;
    ...
    /*
    foreach branch in passed branches
      check if branch exists
      if it does check if post exists in branch
      if does not exist create post

    */
    promises = [];
    branchExistsPromises = [];
    createPromises = [];
    for (let i = 0; i < repostTo.length; i += 1) {

        const branchid = repostTo[i].label;
        const branchExistsPromise = Models.Branch.findById(branchid)
            .then(instance => {
                if (instance === null) {
                ...
                {
                    const promise = Models.Post.findByPostAndBranchIds(postid, branchid)
                        .then(instance => {
                            if (instance === null) {
                                //create post if it does not exist already
                                createPromise = Models.Post.create({
                                    branchid,
                                    comment_count: 0,
    ...
    //not returning done
    return Promise.all(branchExistsPromises).then(() => Promise.all(promises)).
    then(() => Promise.all(createPromises)).then(() => { return next() });
};
